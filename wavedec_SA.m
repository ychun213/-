...
clc;clear;close all
filename='E:\test\Z_RADR_I_Z9250_20160606003100_O_DOR_SA_CAP.bin';
...
ind=isnan(data1_SA);
data1_SA(ind)=0;
% [CA,CH,CV,CD]=dwt2(data1_SA,'db1');%提取一层分解的小波系数    网址分析https://www.cnblogs.com/xixixing/category/862598.html
% YA=idwt2(CA,[],[],[],'db1');
[CA,CH,CV,CD]=swt2(data1_SA,1,'db1');%swt为多孔算法的非抽取小波变换，dwt为常规Mallat算法（每次分解后长度减小一半)
mzeros=zeros(size(CH));
YA=iswt2(CA,mzeros,mzeros,mzeros,'db1');
figure;DrawPicture(X,Y,data1_SA)
figure;DrawPicture(X,Y,YA);
